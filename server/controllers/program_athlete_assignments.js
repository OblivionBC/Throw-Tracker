...
exports.assignProgramToAthletes = asyncHandler(async (req, res) => {
    ...
    for (const assignment of assignments) {
      ...
      if (!trpe_rk) {
        throw new ValidationError(
          "Training period ID is required for each assignment"
        );
      }
      ...
      if (trainingPeriodCheck.rows.length === 0) {
        throw new NotFoundError(`Training period ${trpe_rk}`);
      }
...
exports.getProgramAssignments = asyncHandler(async (req, res) => {
  const { prog_rk } = req.params;
...
exports.removeProgramFromAthlete = asyncHandler(async (req, res) => {
...
exports.getAthletePrograms = asyncHandler(async (req, res) => {
  const { athlete_prsn_rk } = req.params;
  const coach_prsn_rk = req.user.id;

  if (!athlete_prsn_rk) {
    throw new ValidationError("Athlete ID is required");
  }
...
exports.getTrainingPeriodPrograms = asyncHandler(async (req, res) => {
