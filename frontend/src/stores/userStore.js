...
const useUserStore = create(
  persist(
    (set, get) => ({
      ...
      handleExpiredToken: () => {
        Logger.log("🚨 Handling expired token - redirecting to login");
        ...
        if (get().isRedirecting) {
          Logger.log("🚨 Already redirecting, skipping duplicate call");
          return;
        }
        ...
        Logger.log("🚨 About to redirect to /login");
        ...
        try {
          ...
          Logger.error("🚨 Immediate redirect failed:", error);
          ...
          setTimeout(() => {
            try {
              ...
              Logger.error("🚨 Delayed redirect failed:", delayedError);
              try {
                Logger.log(
        ...
        setTimeout(() => {
          if (window.location.pathname !== "/login") {
            Logger.log("🚨 Fallback redirect check - still not on login page");
            try {
              window.location.href = "/login";
            } catch (fallbackError) {
