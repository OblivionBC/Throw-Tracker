...
const AthleteEventAssignmentsList = forwardRef(
  ({ athlete_rk, onRefresh }, ref) => {
    ...
    const getAssignmentData = async () => {
      ...
      try {
        Logger.log("Fetching event assignments for athlete:", athlete_rk);
        const response = await athleteEventAssignmentsApi.getByAthlete(
          athlete_rk
        );
        Logger.log("Event assignments response:", response);
        setAssignmentData(response);
      } catch (error) {
        Logger.error("Error fetching athlete event assignments:", error);
        setAssignmentData([]);
      } finally {
        setLoading(false);
      }
    ...
    const handleDeleteSelected = async () => {
      ...
      try {
        ...
        Logger.error("Error deleting event assignments:", error);
